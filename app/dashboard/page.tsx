...
export default function Dashboard() {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (data.success) {
              ...
              console.error("Job search failed:", data.debug);
              setDebug(data.debug);
            }
          } else {
            console.warn("No profile found, redirecting to setup...");
  ...
  const handleSmartApply = async (job: any) => {
    console.log("Smart Apply clicked for:", job.job_title);
    setSelectedJob(job);
    ...
    try {
      ...
      if (data.success) {
        ...
        console.error("Cover letter generation failed:", data.message);
        setCoverLetter("Error generating cover letter. Please try again.");
      }
    } catch (e) { 
      console.error("Connection error during cover letter generation:", e);
      setCoverLetter("Connection error. Please check your internet.");
    }
  ...
  const confirmApplication = async () => {
    ...
    console.log("Confirming application for:", selectedJob.job_title);
    setIsApplying(true);
    try {
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-72 bg-white border-r border-slate-200 p-8 flex flex-col fixed h-full">
        ...
        <div className="pt-8 border-t border-slate-100 space-y-2">
          ...
          <button onClick={() => auth.signOut().then(() => window.location.href = '/')} className="w-full flex items-center gap-3 p-4 text-red-500 hover:bg-red-50 rounded-2xl font-bold transition-all text-left">
        ...
        <div className="grid grid-cols-3 gap-8">
          ...
          <div className="col-span-2 space-y-8">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => window.location.reload()} className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-2xl font-bold text-slate-600 hover:bg-slate-50 transition-all">
                ...
                {jobs.map((job: any, i: number) => (
                  <div key={i} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:border-blue-300 transition-all group">
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-50 flex justify-end gap-3">
                      <button onClick={() => handleSmartApply(job)} className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-100">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden">
            ...
            <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-slate-900 flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-blue-600" /> Interview Availability
                </h3>
                <div className="flex gap-4">
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
              ...
              <button onClick={confirmApplication} disabled={isApplying || isGeneratingCL} className="flex-[2] py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 flex items-center justify-center gap-2">
