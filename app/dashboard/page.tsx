...
export default function Dashboard() {
  ...
  if (isLoading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><Loader2 className="w-12 h-12 text-blue-600 animate-spin" /></div>;

  return (
    <div className="min-h-screen bg-slate-50 flex relative">
      <aside className="w-72 bg-white border-r border-slate-200 p-8 flex flex-col fixed h-full z-10">
        ...
        <div className="grid grid-cols-3 gap-8">
          <div className="col-span-1">
            <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
              <div className="flex justify-between items-center"><h2 className="font-bold text-xl">Target Profile</h2><button onClick={() => window.location.href = '/profile/edit'} className="text-blue-600 font-bold text-sm">Edit Profile</button></div>
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl"><Briefcase className="text-blue-600 w-5 h-5" /><div><p className="text-xs font-bold text-slate-400 uppercase">Role</p><p className="font-bold text-slate-900">{profile?.targetRole}</p></div></div>
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl"><MapPin className="text-blue-600 w-5 h-5" /><div><p className="text-xs font-bold text-slate-400 uppercase">Location</p><p className="font-bold text-slate-900">{profile?.location}</p></div></div>
              </div>
              <div className="pt-4"><p className="text-xs font-bold text-slate-400 uppercase mb-3">Key Skills</p><div className="flex flex-wrap gap-2">{profile?.skills?.map((s: any, i: any) => (<span key={i} className="px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-sm font-bold">{s}</span>))}</div></div>
            </section>
          </div>
