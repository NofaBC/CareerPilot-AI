...
export default function Dashboard() {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            const res = await fetch('/api/search-jobs', {
              ...
              body: JSON.stringify({ profile: pData, query: pData.targetRole, location: pData.location })
            });
            const data = await res.json();
            if (data.success) setJobs(data.jobs || []);
            else setDebug(data.debug);
          } else {
  ...
  const handleSmartApply = async (job: any) => {
    ...
    try {
      ...
      else setCoverLetter("Error generating cover letter. Please try again.");
    } catch (e) { 
      console.error(e);
